...
{
    internal class HeatExchangerWordProvider : IWordProvider
    {
        public void MakeWord(string filePath, ICalculatedData calculatedData)
        {
            if (calculatedData is not HeatExchangerCalculatedData data)
                throw new NullReferenceException();

            var dataIn = (HeatExchangerInputData) data.InputData;
...

            {
                ...
                var a = dataIn.FirstTubePlate.TubePlateType switch
                {
                ...
                var b = dataIn.IsDifferentTubePlate
                ...
                var c = dataIn.CompensatorType switch
                {
                    CompensatorType.No => "",
...

            {
                ...
                string type1 = ((int)dataIn.FirstTubePlate.TubePlateType).ToString();

                var type2 = dataIn.FirstTubePlate.IsChamberFlangeSkirt ? "_Butt" : "_Flat";

                string type3 = ((int)dataIn.FirstTubePlate.FlangeFace + 1).ToString();
            ...
            //table
            {
                ...
                if (dataIn.FirstTubePlate.TubePlateType == TubePlateType.WeldedInFlange ||
                    dataIn.FirstTubePlate.TubePlateType == TubePlateType.BetweenFlange)
                {
                    table.AddRow()
                    .AddCell("Материал:")
                ...
                if (dataIn.FirstTubePlate.FixTubeInTubePlate == FixTubeInTubePlateType.OnlyRolling ||
                    dataIn.FirstTubePlate.FixTubeInTubePlate == FixTubeInTubePlateType.RollingWithWelding)
                {
                    table.AddRow()
                ...
                if ((dataIn.FirstTubePlate.FixTubeInTubePlate == FixTubeInTubePlateType.OnlyRolling ||
                    dataIn.FirstTubePlate.FixTubeInTubePlate == FixTubeInTubePlateType.RollingWithWelding) &&
                    (dataIn.FirstTubePlate.TubeRolling == TubeRollingType.RollingWithoutGroove ||
                    dataIn.FirstTubePlate.TubeRolling == TubeRollingType.RollingWithOneGroove))
                {
                    table.AddRow()
            ...
            {
                body.AddParagraph("")
                .AppendEquation($"l_пр=l={dataIn.l} мм");
            }
            else
            {
                body.AddParagraph("")
                .AppendEquation($"l_пр=L_1R/3={dataIn.l1R}/3={data.lpr:f2} мм");
            }
            ...
            if (dataIn.FirstTubePlate.IsNeedCheckHardnessTubePlate)
            {
                ...
                body.AddParagraph($"[W]={data.W_d} мм - допустимоее значение прогиба трубной решетки по таблице 2 ГОСТ 34233.7");
