...
{
    internal class ConicalShellWordProvider : IWordProvider
    {
        public void MakeWord(string filePath, ICalculatedData calculatedData)
        {
            if (calculatedData is not ConicalShellCalculatedData data)
                throw new NullReferenceException();

            var dataIn = (ConicalShellInputData)data.InputData;
                ...
                case ConicalConnectionType.WithRingPicture25b:
                    ...
                    body.AddParagraph("")
                        .AppendEquation("a_1p=0.7√(D/cosα_1∙(s_1-c))" +
                                        $"=0.7√({dataIn.D}/cos{dataIn.alfa1}({dataIn.s1}-{data.c:f2}))={data.a1p:f2}");
                    body.AddParagraph("")
                ...
                case ConicalConnectionType.Toroidal:
                    ...
                    body.AddParagraph("")
                        .AppendEquation("a_1p=0.7√(D/cosα_1∙(s_T-c))" +
                                        $"=0.7√({dataIn.D}/cos{dataIn.alfa1}({dataIn.sT}-{data.c:f2}))={data.a1p:f2}");
                    body.AddParagraph("")
            ...
            if (dataIn.IsConnectionWithLittle)
            {
                body.AddParagraph("- для конических и цилиндрических обечаек или штуцера")
                    .AppendEquation("a_1p=√(D_1/cosα_1∙(s_1-c))" +
                                    $"=√({dataIn.D1}/cos{dataIn.alfa1}({dataIn.s1}-{data.c:f2}))={data.a1p_l:f2}");
                body.AddParagraph("")
            ...
            if (dataIn.ConnectionType != ConicalConnectionType.Toroidal)
            {
                ...
                body.AddParagraph("")
                    ...
                    $"={dataIn.D}-1.4∙{data.a1p:f2}∙sin{dataIn.alfa1}={data.Dk:f2} мм");
            }
            else
            {
                body.Elements<Paragraph>().Last()
                ...
                body.AddParagraph("")
                    ...
                    $"={dataIn.D}-2[{dataIn.r}(1-cos{dataIn.alfa1})+0.7∙{data.a1p:f2}∙sin{dataIn.alfa1}]={data.Dk:f2} мм");
            }
            ...
            if (dataIn.IsPressureIn)
            {
                body.AddParagraph("")
                    .AppendEquation("s_k.p=(p∙D_k)/(2∙φ_p∙[σ]-p)(1/cosα_1)" +
                                    $"=({dataIn.p}∙{data.Dk:f2})/(2∙{dataIn.fi}∙{dataIn.SigmaAllow}-{dataIn.p})(1/cos{dataIn.alfa1})={data.s_p:f2} мм");
            }
            else
            {
                body.AddParagraph("")
                ...
                body.AddParagraph("Эффективные размеры конической обечайки вычисляют по формулам (предварительно принимаем ")
                    ...
                    .AddRun("):");
                body.AddParagraph("")
                    .AppendEquation("l_E=(D-D_1)/(2∙sinα_1)" + $"=({dataIn.D}-{dataIn.D1})/(2∙sin{dataIn.alfa1})={data.lE:f2} мм");
                body.AddParagraph("")
                    .AppendEquation("D_E=max{(D+D_1)/(2∙cosα_1);D/cosα_1-0.3∙(D+D_1)∙√((D+D_1)/((s_k-c)∙100)∙tgα_1}");
                body.AddParagraph("")
                    .AppendEquation("(D+D_1)/(2∙cosα_1)" + $"=({dataIn.D}+{dataIn.D1})/(2∙cos{dataIn.alfa1})={data.DE_1:f2}");
                body.AddParagraph("")
                    .AppendEquation("D/cosα_1-0.3∙(D+D_1)∙√((D+D_1)/((s_k-c)∙100)∙tgα_1" +
                    $"{dataIn.D}/cos{dataIn.alfa1}-0.3∙({dataIn.D}+{dataIn.D1})∙√(({dataIn.D}+{dataIn.D1})/(({dataIn.s}-{data.c:f2})∙100)∙tg{dataIn.alfa1}={data.DE_2:f2}");
                body.AddParagraph("")
                ...
                body.AddParagraph("")
                    .AppendEquation($"(1.2∙{dataIn.p}∙{data.Dk:f2})/(2∙{dataIn.fi}∙{dataIn.SigmaAllow}-{dataIn.p})(1/cos{dataIn.alfa1})={data.s_p_2:f2}");
                body.AddParagraph("")
            ...
            if (dataIn.IsPressureIn)
            {
                ...
                body.AddParagraph("")
                    .AppendEquation("[p]=(2∙[σ]∙φ_p∙(s_k-c))/(D_k/cosα_1+(s_k-c))"
                                    + $"=(2∙{dataIn.SigmaAllow}∙{dataIn.fi}∙({dataIn.s}-{data.c:f2}))/({data.Dk:f2}/cos{dataIn.alfa1}+({dataIn.s}-{data.c:f2}))={data.p_d:f2} МПа");
            }
            else
            {
                ...
                body.AddParagraph("")
                    .AppendEquation("[p]_П=(2∙[σ]∙φ_p∙(s_k-c))/(D_k/cosα_1+(s_k-c))" +
                                    $"=(2∙{dataIn.SigmaAllow}∙{dataIn.fi}∙({dataIn.s}-{data.c:f2}))/({data.Dk:f2}/cos{dataIn.alfa1}+({dataIn.s}-{data.c:f2}))={data.p_dp:f2} МПа");
            ...
            if (dataIn.ConnectionType != ConicalConnectionType.WithoutConnection)
            {
                switch (dataIn.ConnectionType)
                {
                    case ConicalConnectionType.Simply:
                        ...
                        body.AddParagraph("")
                            .AppendEquation($"β=0.4√({dataIn.D}/({dataIn.s2}-{data.c:f2}))∙tg{dataIn.alfa1}/(1+√((1+{data.chi_1:f2} (({dataIn.s1}-{data.c:f2})/({dataIn.s2}-{data.c:f2}))^2)/(2∙cos{dataIn.alfa1}){data.chi_1:f2}({dataIn.s1}-{data.c:f2})/({dataIn.s2}-{data.c:f2})))-0.25={data.beta:f2}");
                        body.AddParagraph("")
