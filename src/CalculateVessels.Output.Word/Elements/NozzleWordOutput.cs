...
internal class NozzleWordOutput : IWordOutputElement<NozzleCalculated>
{
    public void MakeWord(string filePath, ICalculatedElement calculatedData)
    {
        if (calculatedData is not NozzleCalculated data)
            throw new NullReferenceException();

        var nozzleDataIn = (NozzleInput)calculatedData.InputData;
        ...
        InsertHeader(body, nozzleDataIn, shellDataIn);

        InsertInitialDataTableShort(body, nozzleDataIn, shellDataIn);

        InsertImage(mainPart, nozzleDataIn);

        InsertInitialDataTableDetailed(body, nozzleDataIn, shellDataIn, data);

        InsertCommonDataCalculated(body, nozzleDataIn, data.CommonData, shellDataIn);

        InsertLoadingConditionsDataCalculated(body, nozzleDataIn, data, shellDataIn);

        InsertConditionsUseFormulas(body, nozzleDataIn, data.CommonData, shellDataIn);
        ...
        //        {
        //            var ellH = ((EllipticalShellInput)shellDataIn).EllipseH;
        ...
        //        {
        //            //TODO
        //            var ellR = ((EllipticalShellCalculated)nozzleDataIn.ShellCalculatedData).EllipseR;
        ...
        //if (shellDataIn is { ShellType: ShellType.Elliptical, IsPressureIn: true })
        //{
        //    body.AddParagraph()
        ...
        //                    $"=({shellDataIn.p}({nozzleDataIn.d}+2∙{nozzleDataIn.cs}))/(2∙{nozzleDataIn.phi1}∙{nozzleDataIn.SigmaAllow1}-{shellDataIn.p})={data.s1p:f2} мм");
        ...
        //    body.AddParagraph()
        //        .AppendEquation(
        //            $"[p]=(2∙{data.K1}∙{nozzleDataIn.phi}∙{data.SigmaAllowShell}∙({shellDataIn.s}-{data.c:f2})∙{data.V:f2})/({data.Dp:f2}+({shellDataIn.s}-{data.c:f2})∙{data.V:f2})={data.p_d:f2} МПа");
        //}
        //else
        //{
        //    body.AddParagraph()
        //        .AppendEquation(
        //            $"[p]_p=(2∙{data.K1}∙{nozzleDataIn.phi}∙{data.SigmaAllowShell}∙({shellDataIn.s}-{data.c:f2})∙{data.V:f2})/({data.Dp}+({shellDataIn.s}-{data.c:f2})∙{data.V:f2})={data.p_dp:f2} МПа");
        //    body.AddParagraph()
        //        .AppendEquation("[p]_E")
        //        .AddRun(
    ...
    private static void InsertConditionsUseFormulas(Body body, NozzleInput nozzleDataIn, NozzleCalculatedCommon cdc, ShellInputData shellDataIn)
    {
        ...
        switch (shellDataIn.ShellType)
        {
            case ShellType.Cylindrical:
                body.AddParagraph()
                    .AppendEquation("(d_p-2∙c_s)/D" +
                                    $"=({cdc.dp:f2}-2∙{nozzleDataIn.cs})/{shellDataIn.D}={cdc.ConditionUseFormulas1:f2}≤1");
                body.AddParagraph()
                    .AppendEquation("(s-c)/D" +
                                    $"=({shellDataIn.s}-{cdc.c:f2})/({shellDataIn.D})={cdc.ConditionUseFormulas2:f2}≤0.1");
                break;
            case ShellType.Conical:
                body.AddParagraph()
                        .AppendEquation("(d_p-2∙c_s)/D_K" +
                                        $"=({cdc.dp:f2}-2∙{nozzleDataIn.cs})/{cdc.Dk}={cdc.ConditionUseFormulas1:f2}≤1");
                body.AddParagraph()
                    .AppendEquation("(s-c)/D_K≤0.1/cosα");
                body.AddParagraph()
                    .AppendEquation($"({shellDataIn.s}-{cdc.c:f2})/({cdc.Dk})={cdc.ConditionUseFormulas2:f2}");
                body.AddParagraph()
                    .AppendEquation($"0.1/cos{cdc.alpha1}");
                break;
            case ShellType.Elliptical:
            case ShellType.Spherical:
            case ShellType.Torospherical:
                ...
                body.AddParagraph()
                    .AppendEquation("(d_p-2∙c_s)/D " +
                                    $"=({cdc.dp:f2}-2∙{nozzleDataIn.cs})/{shellDataIn.D}={cdc.ConditionUseFormulas1:f2}≤0.6");
                body.AddParagraph()
                    .AppendEquation("(s-c)/D" +
                                    $"=({shellDataIn.s}-{cdc.c:f2})/({shellDataIn.D})={cdc.ConditionUseFormulas2:f2}≤0.1");
                break;
    ...
    private static void InsertOneLoadingConditionDataCalculated(Body body, NozzleInput nozzleDataIn, NozzleCalculatedOneLoading data, NozzleCalculatedCommon cdc, ShellInputData shellDataIn, bool withNumber = false)
    {
        ...
        if (shellDataIn is { ShellType: ShellType.Elliptical } && loadingCondition.PressureType == PressureType.Inside)
        {
            body.AddParagraph()
                .AppendEquation("s_p=(p∙D_p)/(4∙φ∙[σ]-p)" +
                                $"=({loadingCondition.p}∙{cdc.Dp:f2})/(4∙{shellDataIn.phi}∙{data.SigmaAllowShell}-{loadingCondition.p})={data.sp:f2} мм");
        }
        else
        {
            body.Elements<Paragraph>().Last().AddRun(" определяется в соответствии с ГОСТ 34233.2");
        ...
        switch (shellDataIn.ShellType)
        {
            ...
            case ShellType.Conical:
                {
                    body.AddParagraph()
                        .AppendEquation($"d_max=D_K={cdc.dmax:f2} мм")
                        .AddRun(" - для отверстий в конических обечайках");
                    break;
                }
            case ShellType.Elliptical:
            ...
            case ShellType.Torospherical:
                {
                    body.AddParagraph()
                        .AppendEquation($"d_max=0.6∙D={cdc.dmax:f2} мм")
                        .AddRun(" - для отверстий в выпуклых днищах");
                    break;
                }
        }
        ...
        else
        {
            ...
            body.AddParagraph()
                .AppendEquation(
                    $"p_pn={loadingCondition.p}/√(1-({loadingCondition.p}/{data.pen:f2})^2)={data.ppn:f2} МПа");
            body.AddParagraph()
                .AppendEquation($"s_pn=({data.ppn:f2}∙{cdc.Dp:f2})/(2∙{cdc.K1}∙{data.SigmaAllowShell}-{data.ppn:f2})={data.spn:f2} мм");
        }

        body.AddParagraph()
            .AppendEquation(
                $"2∙((s-c)/s_pn-0.8)∙√(D_p∙(s-c))=2∙(({shellDataIn.s}-{cdc.c:f2})/{data.spn:f2}-0.8)∙√({cdc.Dp:f2}∙({shellDataIn.s}-{cdc.c:f2}))={data.d01:f2}");
        body.AddParagraph()
            .AppendEquation($"d_max+2∙c_s={cdc.dmax:f2}+2∙{nozzleDataIn.cs}={data.d02:f2}");
        body.AddParagraph()
            .AppendEquation($"d_0=min({data.d01:f2};{data.d02:f2})={data.d0:f2} мм");

        body.AddParagraph().AppendEquation($"{cdc.dp:f2}≤{data.d0:f2}");
        if (cdc.dp <= data.d0)
        {
            body.AddParagraph("Условие прочности выполняется").Bold();
            body.Elements<Paragraph>().Last()
        ...
        else
        {
            ...
            body.AddParagraph()
                .AppendEquation(
                    $"0.5∙(d_p-d_0p)∙s_p=0.5∙({cdc.dp:f2}-{cdc.d0p:f2})∙{data.sp:f2}={data.ConditionStrengthening2:f2}");
            body.AddParagraph()
                .AppendEquation($"{data.ConditionStrengthening1:f2}≥{data.ConditionStrengthening2:f2}");
            if (data.ConditionStrengthening1 >= data.ConditionStrengthening2)
            {
        ...
        body.AddParagraph()
            .AppendEquation(
                $"(s_0-c)/(s-c)=({nozzleDataIn.s0}-{cdc.c:f2})/({shellDataIn.s}-{cdc.c:f2})={data.V1:f2}");
        body.AddParagraph()
            .AppendEquation(
                "(χ_4+(l_1p∙(s_1-c_s)∙χ_1+l_2p∙s_2∙χ_2+l_3p∙(s_3-c_s-c_s1)∙χ_3)/(l_p∙(s-c)))/(1+0.5∙(d_p-d_0p)/l_p+K_1∙(d+2∙c_s)/D_p∙(φ/φ_1)∙(l_1p/l_p))=");
        body.AddParagraph()
            .AppendEquation(
                $"({data.psi4:f2}+({cdc.l1p:f2}∙({nozzleDataIn.s1}-{nozzleDataIn.cs})∙{data.psi1:f2}+{cdc.l2p:f2}∙{nozzleDataIn.s2}∙{data.psi2:f2}+{cdc.l3p:f2}∙({nozzleDataIn.s3}-{nozzleDataIn.cs}-{nozzleDataIn.cs1})∙{data.psi3:f2})/({cdc.lp:f2}∙({shellDataIn.s}-{cdc.c:f2})))/(1+0.5∙({cdc.dp:f2}-{cdc.d0p:f2})/{cdc.lp:f2}+{cdc.K1}∙({nozzleDataIn.d}+2∙{nozzleDataIn.cs})/{cdc.Dp:f2}∙({nozzleDataIn.phi}/{nozzleDataIn.phi1})∙({cdc.l1p:f2}/{cdc.lp:f2}))={data.V2:f2}");
        ...
        if (loadingCondition.PressureType == PressureType.Inside)
        {
            body.AddParagraph()
                .AppendEquation(
                    $"[p]=(2∙{cdc.K1}∙{nozzleDataIn.phi}∙{data.SigmaAllowShell}∙({shellDataIn.s}-{cdc.c:f2})∙{data.V:f2})/({cdc.Dp:f2}+({shellDataIn.s}-{cdc.c:f2})∙{data.V:f2})={data.p_d:f2} МПа");
        }
        else
        {
            body.AddParagraph()
                .AppendEquation(
                    $"[p]_p=(2∙{cdc.K1}∙{nozzleDataIn.phi}∙{data.SigmaAllowShell}∙({shellDataIn.s}-{cdc.c:f2})∙{data.V:f2})/({cdc.Dp}+({shellDataIn.s}-{cdc.c:f2})∙{data.V:f2})={data.p_dp:f2} МПа");
            body.AddParagraph()
                .AppendEquation("[p]_E")
                .AddRun(
                    " - допускаемое наружное давление из условия устойчивости в пределах упругости, определяемое по ГОСТ 34233.2 для соответствующих обечайки и днища без отверстий");
    ...
    private static void InsertCommonDataCalculated(Body body, NozzleInput nozzleDataIn, NozzleCalculatedCommon cdc, ShellInputData shellDataIn)
    {
        body.AddParagraph("Расчетные параметры").Alignment(AlignmentType.Center);
        body.AddParagraph();
        ...
        switch (shellDataIn.ShellType)
        {
            ...
            case ShellType.Elliptical:
                {
                    ...
                    if (Math.Abs(ellH * 100 - shellDataIn.D * 25) < 0.00001)
                    {
                        ...
                        body.AddParagraph()
                            .AppendEquation("D_p=2∙D∙√(1-3∙(x/D)^2)" +
                                            $"=2∙{shellDataIn.D}∙√(1-3∙({nozzleDataIn.ellx}/{shellDataIn.D})^2)={cdc.Dp:f2} мм");
                    }
                    else
                    {
                        body.Elements<Paragraph>().Last()
                        ...
                        body.AddParagraph()
                            .AppendEquation("D_p=D^2/(2∙H)∙√(1-(D^2-4∙H^2)/D^4∙x^2)" +
                                            $"={shellDataIn.D}^2/(2∙{ellH})∙√(1-({shellDataIn.D}^2-4∙{ellH}^2)/{shellDataIn.D}^4∙{nozzleDataIn.ellx}^2)={cdc.Dp:f2} мм");
                    }
            ...
            case ShellType.Torospherical:
                {
                    //TODO
                    var ellR = ((EllipticalShellCalculated)nozzleDataIn.ShellCalculatedData).CommonData.EllipseR;
            ...
            case NozzleLocation.LocationAccordingToParagraph_5_2_2_1:
                {
                    ...
                    body.AddParagraph()
                        .AppendEquation("d_p=d+2∙c_s" +
                                        $"={nozzleDataIn.d}+2∙{nozzleDataIn.cs}={cdc.dp:f2} мм");
                    break;
                }
            case NozzleLocation.LocationAccordingToParagraph_5_2_2_2:
                body.AddParagraph(
        ...
        body.AddParagraph()
            .AppendEquation(
                $"1.25√((d+2∙c_s)(s_1-c_s))=1.25√(({nozzleDataIn.d}+2∙{nozzleDataIn.cs})({nozzleDataIn.s1}-{nozzleDataIn.cs}))={cdc.l1p2:f2} мм");
        body.AddParagraph().AppendEquation($"l_1p=min({nozzleDataIn.l1};{cdc.l1p2:f2})={cdc.l1p:f2} мм");
        ...
        if (nozzleDataIn.l3 > 0)
        {
            ...
            body.AddParagraph()
                .AppendEquation(
                    $"0.5√((d+2∙c_s)(s_3-c_s-c_s1))=0.5√(({nozzleDataIn.d}+2∙{nozzleDataIn.cs})({nozzleDataIn.s3}-{nozzleDataIn.cs}-{nozzleDataIn.cs1}))={cdc.l3p2:f2} мм");
            body.AddParagraph().AppendEquation($"l_3p=min({nozzleDataIn.l3};{cdc.l3p2:f2})={cdc.l3p:f2} мм");
        }

        body.AddParagraph("Ширина зоны укрепления отверстия в цилиндрической обечайке");
        body.AddParagraph()
            .AppendEquation("L_0=√(D_p∙(s-c))" +
                            $"=√({cdc.Dp}∙({shellDataIn.s}-{cdc.c:f2}))={cdc.L0:f2}");
        ...
        switch (nozzleDataIn.NozzleKind)
        {
            ...
            case NozzleKind.WithRingAndInPart:
            case NozzleKind.WithFlanging:
                body.AddParagraph().AppendEquation($"l_p=L_0={cdc.lp:f2} мм");
                break;
            case NozzleKind.WithTorusshapedInsert:
            case NozzleKind.WithWealdedRing:
                body.AddParagraph()
                    .AppendEquation($"l_p=min{{l;L_0}}=min{{{nozzleDataIn.l};{cdc.L0:f2}}}={cdc.lp:f2} мм");
                break;
        }
        ...
        if (nozzleDataIn.l2 > 0)
        {
            ...
            body.AddParagraph()
                .AppendEquation(
                    $"√(D_p∙(s_2+s-c))=√({cdc.Dp:f2}∙({nozzleDataIn.s2}+{shellDataIn.s}-{cdc.c:f2}))={cdc.l2p2:f2} мм");
            body.AddParagraph().AppendEquation($"l_2p=min({nozzleDataIn.l2};{cdc.l2p2:f2})={cdc.l2p:f2} мм");
        }

        var typeString = shellDataIn.ShellType switch
        {
            ShellType.Cylindrical => " цилиндрической обечайки ",
            ShellType.Elliptical => " выпуклого днища ",
            ShellType.Conical => " конической обечайки ",
            ShellType.Spherical => " выпуклого днища ",
            ShellType.Torospherical => " выпуклого днища ",
            _ => throw new ArgumentOutOfRangeException()
        };

        body.AddParagraph("Расчетный диаметр отверстия, не требующий укрепления в стенке" + typeString +
                          "при отсутствии избыточной толщины стенки сосуда и при наличии штуцера");

        body.AddParagraph()
            .AppendEquation("d_0p=0,4√(D_p∙(s-c))" +
                            $"=0.4√({cdc.Dp}∙({shellDataIn.s}-{cdc.c:f2}))={cdc.d0p:f2} мм");
    }

    private static void InsertHeader(Body body, NozzleInput nozzleDataIn, ShellInputData shellDataIn)
    {
    ...
    private static void InsertInitialDataTableDetailed(Body body, NozzleInput nozzleDataIn, ShellInputData shellDataIn, NozzleCalculated data)
    {
        ...
        table.AddRow()
            ...
            .AddCell($"{data.CommonData.c:f2} мм");
        ...
        table.AddRow()
            ...
            .AppendEquation("φ_1")
            .AppendText(":")
            .AddCell($"{nozzleDataIn.phi1}");

        table.AddRow()
            .AddCell("Шов обечайки в зоне врезки штуцера ")
            .AppendEquation("φ")
            .AppendText(":")
            .AddCell($"{nozzleDataIn.phi}");

        var loadingConditions = ((ShellInputData)nozzleDataIn.ShellCalculatedData.InputData).LoadingConditions.ToList();

        WordHelpers.AddLoadingConditionsInTableForShells(loadingConditions, table);
    ...
    private static void AddMaterialCharacteristic(Table table, double sigmaAllow, double EAllow, double temperature, int index)
    {
        table.AddRow()
            .AddCell($"Допускаемое напряжение при расчетной температуре {temperature} °С, ")
            .AppendEquation($"[σ]_{index}")
            .AppendText(":")
            .AddCell($"{sigmaAllow} МПа");

        table.AddRow()
            .AddCell($"Модуль продольной упругости при расчетной температуре {temperature} °С, ")
            .AppendEquation($"E_{index}")
            .AppendText(":")
            .AddCell($"{EAllow} МПа");
    }

    private static void InsertImage(MainDocumentPart mainPart, NozzleInput nozzleDataIn)
    {
        byte[] bytes = (byte[])(Data.Properties.Resources.ResourceManager.GetObject($"Nozzle{(int)nozzleDataIn.NozzleKind}")
                                ?? throw new InvalidOperationException());

        mainPart.InsertImage(bytes, ImagePartType.Gif);
    }

    private static void InsertInitialDataTableShort(Body body, NozzleInput nozzleDataIn, ShellInputData shellDataIn)
    {
