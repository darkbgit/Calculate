...
internal class ConicalShellWordOutput : IWordOutputElement<ConicalShellCalculated>
{
    public void MakeWord(string filePath, ICalculatedElement calculatedData)
    {
        ...
        if (dataIn.IsPressureIn)
        {
            body.AddParagraph()
                .AppendEquation("s_(k.p)=(p∙D_k)/(2∙φ_p∙[σ]-p)(1/cosα_1)" +
                                $"=({dataIn.p}∙{data.Dk:f2})/(2∙{dataIn.phi}∙{data.SigmaAllow}-{dataIn.p})(1/cos{RadiansToDegree(data.alpha1):f0})={data.s_p:f2} мм");
        }
        else
        {
            body.AddParagraph()
            ...
            body.AddParagraph("Эффективные размеры конической обечайки вычисляют по формулам (предварительно принимаем ")
                ...
                .AddRun("):");
            body.AddParagraph()
                .AppendEquation("l_E=(D-D_1)/(2∙sinα_1)" + $"=({dataIn.D}-{dataIn.D1})/(2∙sin{RadiansToDegree(data.alpha1)})={data.lE:f2} мм");
            body.AddParagraph()
                .AppendEquation("D_E=max{(D+D_1)/(2∙cosα_1);D/cosα_1-0.3∙(D+D_1)∙√((D+D_1)/((s_k-c)∙100)∙tgα_1}");
            body.AddParagraph()
                .AppendEquation("(D+D_1)/(2∙cosα_1)" + $"=({dataIn.D}+{dataIn.D1})/(2∙cos{RadiansToDegree(RadiansToDegree(data.alpha1)):f0})={data.DE_1:f2}");
            body.AddParagraph()
                .AppendEquation("D/cosα_1-0.3∙(D+D_1)∙√((D+D_1)/((s_k-c)∙100)∙tgα_1" +
                                $"{dataIn.D}/cos{RadiansToDegree(data.alpha1):f0}-0.3∙({dataIn.D}+{dataIn.D1})∙√(({dataIn.D}+{dataIn.D1})/(({dataIn.s}-{data.c:f2})∙100)∙tg{RadiansToDegree(data.alpha1)}={data.DE_2:f2}");
            body.AddParagraph()
            ...
            body.AddParagraph()
                .AppendEquation($"(1.2∙{dataIn.p}∙{data.Dk:f2})/(2∙{dataIn.phi}∙{data.SigmaAllow}-{dataIn.p})(1/cos{RadiansToDegree(data.alpha1):f0})={data.s_p_2:f2}");
            body.AddParagraph()
        ...
        CheckCalculatedThickness("s_k", dataIn.s, data.s_p, body);
        ...
        if (dataIn.IsPressureIn)
        {
            ...
            body.AddParagraph()
                .AppendEquation("[p]=(2∙[σ]∙φ_p∙(s_k-c))/(D_k/cosα_1+(s_k-c))"
                                + $"=(2∙{data.SigmaAllow}∙{dataIn.phi}∙({dataIn.s}-{data.c:f2}))/({data.Dk:f2}/cos{RadiansToDegree(data.alpha1):f0}+({dataIn.s}-{data.c:f2}))={data.p_d:f2} МПа");
        }
        else
        {
            ...
            body.AddParagraph()
                .AppendEquation("[p]_П=(2∙[σ]∙φ_p∙(s_k-c))/(D_k/cosα_1+(s_k-c))" +
                                $"=(2∙{data.SigmaAllow}∙{dataIn.phi}∙({dataIn.s}-{data.c:f2}))/({data.Dk:f2}/cos{RadiansToDegree(data.alpha1):f0}+({dataIn.s}-{data.c:f2}))={data.p_dp:f2} МПа");
        ...
        CheckCalculatedPressure(dataIn.p, data.p_d, body);
        ...
        {
            body.AddParagraph()
                .AppendEquation($"α_1={RadiansToDegree(data.alpha1):f0}≤70");
        }

        body.AddParagraph()
            .AppendEquation(
                $"0.001≤(s∙cosα_1)/D=({dataIn.s}∙cos{RadiansToDegree(data.alpha1):f0})/{dataIn.D}={data.ConditionUseFormulas:f3}≤0.05");
        ...
        if (dataIn.ConnectionType != ConicalConnectionType.WithoutConnection)
        {
            ...
            switch (dataIn.ConnectionType)
            {
                case ConicalConnectionType.Simply:
                    ...
                    body.AddParagraph()
                        .AppendEquation(
                            $"β=0.4√({dataIn.D}/({dataIn.s2Big}-{data.c:f2}))∙tg{RadiansToDegree(data.alpha1):f0}/(1+√((1+{data.chi_1Big:f2} (({dataIn.s1Big}-{data.c:f2})/({dataIn.s2Big}-{data.c:f2}))^2)/(2∙cos{RadiansToDegree(data.alpha1):f0}){data.chi_1Big:f2}({dataIn.s1Big}-{data.c:f2})/({dataIn.s2Big}-{data.c:f2})))-0.25={data.beta:f2}");
                    body.AddParagraph()
                        .AppendEquation($"β_1=max{{0.5;{data.beta:f2}}}={data.beta_1:f2}");
                    body.AddParagraph()
                        .AppendEquation(
                            $"s_2p=({dataIn.p}∙{dataIn.D}∙{data.beta_1:f2})/(2∙{data.SigmaAllow}∙{dataIn.phi}-{dataIn.p})={data.s_2pBig:f2} мм");

                    body.AddParagraph()
                        .AppendEquation($"s_2={data.s_2pBig:f2}+{data.c:f2}={data.s_2Big:f2} мм");

                    CheckCalculatedThickness("s_2", dataIn.s2Big, data.s_2Big, body);

                    body.AddParagraph()
                        .AppendEquation("s_1=(s_1-c)/(s_2-c)s_2p+c=" +
                                    $"({dataIn.s1Big}-{data.c})/({dataIn.s2Big}-{data.c}){data.s_2pBig:f2}+{data.c:f2}={data.s_1Big:f2} мм");

                    CheckCalculatedThickness("s_1", dataIn.s1Big, data.s_1Big, body);
                    ...
                    body.AddParagraph()
                        .AppendEquation("[p]=(2∙[σ]_2∙φ_p∙(s_2-c))/(D∙β_1+(s_2-c))" +
                                        $"=(2∙{data.SigmaAllow2Big}∙{dataIn.phi}∙({dataIn.s2Big}-{data.c:f2}))/({dataIn.D}∙{data.beta_1:f2}+({dataIn.s2Big}-{data.c:f2}))={data.p_dBig:f2} МПа");
                    break;
                case ConicalConnectionType.WithRingPicture25b:
                    ...
                    body.AddParagraph()
                        .AppendEquation("β_a=((2∙[σ]_2∙φ_p)/p-1)(s_2-c)/D" +
                                        $"((2∙{data.SigmaAllow2Big}∙{dataIn.phi})/{dataIn.p}-1)∙({dataIn.s2Big}-{data.c})/{dataIn.D}={data.beta_a:f2}");
                    body.AddParagraph()
                        .AppendEquation(
                            "β=0.4√(D/(s_2-c))∙tgα_1/(1+√((1+χ_1∙((s_1-c)/(s_2-c))^2)/(2∙cosα_1)χ_1∙(s_1-c)/(s_2-c)))-0.25");
                    body.AddParagraph()
                    ...
                    body.AddParagraph()
                        .AppendEquation(
                            $"β=0.4√({dataIn.D}/({dataIn.s2Big}-{data.c:f2}))∙tg{RadiansToDegree(data.alpha1):f0}/(1+√((1+{data.chi_1Big:f2} (({dataIn.s1Big}-{data.c:f2})/({dataIn.s2Big}-{data.c:f2}))^2)/(2∙cos{RadiansToDegree(data.alpha1)}){data.chi_1Big:f2}({dataIn.s1Big}-{data.c:f2})/({dataIn.s2Big}-{data.c:f2})))-0.25={data.beta:f2}");

                    body.AddParagraph()
                        .AppendEquation($"A_K=({dataIn.p}∙{dataIn.D}^2∙tg{RadiansToDegree(data.alpha1):f0})/(8∙{data.SigmaAllowC}∙{dataIn.phi_k})∙(1-({data.beta_a:f2}+0.25)/({data.beta:f2}+0.25))={data.Ak:f2} мм^2");
                    ...
                    body.AddParagraph()
                        .AppendEquation("B_2=(1.6∙A_K)/((s_2-c)√(D∙(s_2-c)))∙([σ]_K∙φ_K)/([σ]_2∙φ_T)" +
                                        $"(1.6∙{dataIn.Ak})/(({dataIn.s2Big} - {data.c})∙√({dataIn.D}∙({dataIn.s2Big}-{data.c})))∙({data.SigmaAllowC}∙{dataIn.phi_k})/({data.SigmaAllow2Big}∙{dataIn.phi_t})={data.B2:f2}");
                    body.AddParagraph()
                        .AppendEquation("B_3=0.25");

                    body.AddParagraph()
                        .AppendEquation(
                            $"β_0=(0.4√({dataIn.D}/({dataIn.s2Big}-{data.c}))∙tg{RadiansToDegree(data.alpha1):f0}-{data.B3}[1+√((1+{data.chi_1Big}(({dataIn.s1Big}-{data.c})/({dataIn.s2Big}-{data.c}))^2)/(2∙cos{RadiansToDegree(data.alpha1):f0}){data.chi_1Big}({dataIn.s1Big}-{data.c})/({dataIn.s2Big}-{data.c}))])/" +
                            $"({data.B2:f2}+[1+√((1+{data.chi_1Big}(({dataIn.s1Big}-{data.c})/({dataIn.s2Big}-{data.c}))^2)/(2∙cos{RadiansToDegree(data.alpha1):f0}){data.chi_1Big}({dataIn.s1Big}-{data.c})/({dataIn.s2Big}-{data.c}))])={data.beta_0:f2}");
                    ...
                    body.AddParagraph()
                        .AppendEquation(
                            $"[p]=(2∙{data.SigmaAllow2Big}∙{dataIn.phi}∙({dataIn.s2Big}-{data.c}))/({dataIn.D}∙{data.beta_2:f2}+({dataIn.s2Big}-{data.c}))={data.p_dBig:f2} МПа");
                    break;
                case ConicalConnectionType.WithRingPicture29:
                    body.AddParagraph("Соединение обечайки с укрепляющим кольцом");
                    body.AddParagraph("Площадь поперечного сечения укрепляющего кольца вычисляют по формуле");
                    body.AddParagraph()
                        .AppendEquation("A_K=(p∙D^2∙tgα_1)/(8∙[σ]_K∙φ_K)=" +
                                        $"({dataIn.p}∙{dataIn.D}^2∙tg{RadiansToDegree(data.alpha1):f0})/(8∙{data.SigmaAllowC}∙{dataIn.phi_k})={data.Ak:f2} мм^2");
                    ...
                    body.AddParagraph()
                        .AppendEquation("[p]=(8∙[σ]_K∙φ_K)/(D^2∙tgα_1)" +
                                        $"(8∙{data.SigmaAllowC}∙{dataIn.phi_k})/({dataIn.D}^2∙tg{RadiansToDegree(data.alpha1):f0})={data.p_dBig:f2} МПа");
                    break;
                case ConicalConnectionType.Toroidal:
                    ...
                    body.AddParagraph()
                        .AppendEquation(
                            ...
                            $"=1/(1+(0.028∙{RadiansToDegree(data.alpha1):f0}∙{dataIn.r}/{dataIn.D}∙√({dataIn.D}/({dataIn.sT}-{data.c})))/(1/√cos{RadiansToDegree(data.alpha1):f0}+1={data.beta_t:f2}");
                    body.AddParagraph()
                        .AppendEquation($"β_3=max{{0.5;{data.beta:f2}∙{data.beta_t:f2}}}=max{{0.5;{data.beta_3_2:f2}}}={data.beta_3:f2}");
                    body.AddParagraph()
                        .AppendEquation(
                            $"s_(T∙p)=({dataIn.p}∙{dataIn.D}∙{data.beta_3:f2})/(2∙{dataIn.phi}∙{data.SigmaAllowT}-{dataIn.p})={data.s_tp:f2} мм");

                    body.AddParagraph().AppendEquation($"s_T={data.s_tp:f2}+{data.c:f2}={data.s_t:f2} мм");
                    CheckCalculatedThickness("s_T", dataIn.sT, data.s_t, body);
                    ...
                    body.AddParagraph()
                        .AppendEquation("[p]=(2∙[σ]∙φ_p∙(s_T-c))/(D∙β_3+(s_T-c))" +
                                        $"=(2∙{data.SigmaAllowT}∙{dataIn.phi}∙({dataIn.sT}-{data.c:f2}))/({dataIn.D}∙{data.beta_3:f2}+({dataIn.sT}-{data.c:f2}))={data.p_dBig:f2} МПа");
            ...
            CheckCalculatedPressure(dataIn.p, data.p_dBig, body);
            ...
            switch (dataIn.ConnectionType)
            {
                ...
                case ConicalConnectionType.WithRingPicture25b:
                    body.AddParagraph()
                        .AppendEquation($"(s_1-c)=({dataIn.s1Big}-{data.c})={dataIn.s1Big - data.c}≥(s_2-c)=({dataIn.s2Big}-{data.c})={dataIn.s2Big - data.c}");
                    break;
                case ConicalConnectionType.Toroidal:
                    body.AddParagraph()
        ...
        if (dataIn.IsConnectionWithLittle)
        {
            ...
            if (data.ConditionForBetaH >= 1)
            {
                body.AddParagraph()
                    ...
                    $"={data.chi_1Little:f2}(({dataIn.s1Little}-{data.c:f2})/({dataIn.s2Little}-{data.c:f2}))^2={data.ConditionForBetaH:f2}≥1");
                ...
                body.AddParagraph()
                    .AppendEquation(
                        $"β=0.4√({dataIn.D1}/({dataIn.s2Little}-{data.c:f2}))∙tg{RadiansToDegree(data.alpha1):f0}/(1+√((1+{data.chi_1Little:f2} (({dataIn.s1Little}-{data.c:f2})/({dataIn.s2Little}-{data.c:f2}))^2)/(2∙cos{RadiansToDegree(data.alpha1)}){data.chi_1Little:f2}({dataIn.s1Little}-{data.c:f2})/({dataIn.s2Little}-{data.c:f2})))-0.25={data.betaLittle:f2}");
            ...
            else
            {
                ...
                body.AddParagraph()
                    .AppendEquation(
                        $"β_H=0.4√({dataIn.D1}/({dataIn.s2Little}-{data.c:f2}))∙tg{RadiansToDegree(data.alpha1):f0}/({data.chi_1Little}({dataIn.s1Little}-{data.c})/({dataIn.s2Little}-{data.c})∙√(({dataIn.s1Little}-{data.c})/(({dataIn.s2Little}-{data.c})∙cos{RadiansToDegree(data.alpha1):f0}))+√((1+{data.chi_1Little:f2}(({dataIn.s1Little}-{data.c:f2})/({dataIn.s2Little}-{data.c:f2}))^2)/2)+0.5={data.beta_H:f2}");
            ...
            body.AddParagraph()
                .AppendEquation(
                    $"s_2p=({dataIn.p}∙{dataIn.D1}∙{data.beta_4:f2})/(2∙{dataIn.phi}∙{data.SigmaAllow2Little}-{dataIn.p})={data.s_2pLittle:f2} мм");

            body.AddParagraph().AppendEquation($"s_2={data.s_2pLittle:f2}+{data.c:f2}={data.s_2Little:f2} мм");

            CheckCalculatedThickness("s_2", dataIn.s2Little, data.s_2Little, body);

            body.AddParagraph()
                .AppendEquation("s_1=(s_1-c)/(s_2-c)s_2p+c=" +
                                $"({dataIn.s1Little}-{data.c})/({dataIn.s2Little}-{data.c}){data.s_2pLittle}+{data.c:f2}={data.s_1Little:f2} мм");

            CheckCalculatedThickness("s_1", dataIn.s1Little, data.s_1Little, body);
            ...
            body.AddParagraph()
                .AppendEquation("[p]=(2∙[σ]_2∙φ_p∙(s_2-c))/(D_1∙β_4+(s_2-c))" +
                                $"=(2∙{data.SigmaAllow2Little}∙{dataIn.phi}∙({dataIn.s2Little}-{data.c:f2}))/({dataIn.D1}∙{data.beta_4:f2}+({dataIn.s2Little}-{data.c:f2}))={data.p_dLittle:f2} МПа");

            CheckCalculatedPressure(dataIn.p, data.p_dLittle, body);
    ...
    private static void CheckCalculatedPressure(double input, double calculated, Body body)
    {
        body.AddParagraph()
            .AppendEquation("[p]≥p");
        body.AddParagraph()
    ...
    private static void CheckCalculatedThickness(string name, double input, double calculated, Body body)
    {
        if (input > calculated)
        {
    ...
    private static void ReinforcementRingSquareCheck(ConicalShellInput dataIn, ConicalShellCalculated data, Body body)
    {
        if (data.Ak <= 0)
        {
