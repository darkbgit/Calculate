...
internal class FlatBottomWordOutput : IWordOutputElement<FlatBottomCalculated>
{
    ...
    {
        if (calculatedData is not FlatBottomCalculated data)
            throw new NullReferenceException();

        var dataIn = (FlatBottomInput)data.InputData;
        ...
        body.AddParagraph()
            .AppendEquation($"s_1={data.s1p:f2}+{data.c:f2}={data.s1:f2} мм");
        ...
        if (dataIn.Type is 10 or 11 or 12)
        {
            ...
            body.AddParagraph()
                .AppendEquation($"s_2={data.s2p:f2}+{data.c:f2}={data.s2:f2} мм");
