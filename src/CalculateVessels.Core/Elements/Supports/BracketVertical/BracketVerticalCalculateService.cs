...
internal class BracketVerticalCalculateService : ICalculateService<BracketVerticalInput>
{
    private readonly IPhysicalDataService _physicalData;
    public BracketVerticalCalculateService(IPhysicalDataService physicalData)
    {
    ...
    public ICalculatedElement Calculate(BracketVerticalInput dataIn)
    {
        var data = new BracketVerticalCalculated
        {
            InputData = dataIn,
        ...
        data.e1 = dataIn.e1 == 0 ? 5.0 / 6.0 * dataIn.l1 : dataIn.e1;
        ...
        data.x = Math.Log(data.Dp / (2 * (dataIn.s - dataIn.c)));
        ...
        if (!dataIn.ReinforcingPad)
        {
            ...
            data.v2 = -data.sigma_m / (data.K2 * data.SigmaAllow * dataIn.phi);
            ...
            data.sigmaid = data.K1 * data.K2 * data.SigmaAllow;

            data.F1Allow = data.sigmaid * dataIn.h1 *
                Math.Pow(dataIn.s - dataIn.c, 2) / (data.K7 * data.e1e);

            if (dataIn.g / dataIn.h1 < 0.5)
            {
                data.F1Allow *= 0.5 + dataIn.g / dataIn.h1;
            }
        }
        else
        {
        ...
        if (data.F1 > data.F1Allow)
        {
            data.ErrorList.Add("Несущая способность обечайки в месте приварки опорной лапы " +
                               (dataIn.ReinforcingPad ? "с подкладным листом" : "без подкладного листа.") +
