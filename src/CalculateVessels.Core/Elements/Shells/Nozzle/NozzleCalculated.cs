...
public class NozzleCalculated : CalculatedElement, ICalculatedElement
{
    public NozzleCalculated()
    {
        Bibliography = new[]
        {
    ...
    public required IEnumerable<NozzleCalculatedOneLoading> Results { get; init; } =
        new List<NozzleCalculatedOneLoading>();
    ...
    {
        if (InputData is not NozzleInput dataIn) return "Nozzle";

        var builder = new StringBuilder();
        builder.Append("Nozzle - ");
        builder.Append($"{dataIn.d} mm ");
        builder.Append($" in shell - {dataIn.ShellCalculatedData}");
