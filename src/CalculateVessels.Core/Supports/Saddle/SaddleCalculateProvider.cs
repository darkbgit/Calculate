...
{
    internal class SaddleCalculateProvider : ICalculateProvider
    {
        public ICalculatedData Calculate(IInputData inputData)
        {
            if (inputData is not SaddleInputData dataIn)
                throw new CalculateException("Error. Input data wrong type.");

            var data = new SaddleCalculatedData(dataIn);
            ...
            data.ny = dataIn.IsAssembly ? 1.8 : 2.4;
                ...
                case SaddleType.SaddleWithoutRingWithoutSheet:
                    {
                        ...
                        data.K12 = (1.15 - 0.1432 * DegToRad(dataIn.delta1)) /
                                Math.Sin(0.5 * DegToRad(dataIn.delta1));
                        data.K13 = Math.Max(1.7 - 2.1 * DegToRad(dataIn.delta1) / Math.PI, 0) /
                                Math.Sin(0.5 * DegToRad(dataIn.delta1));
                        data.K14 = (1.45 - 0.43 * DegToRad(dataIn.delta1)) /
                                Math.Sin(0.5 * DegToRad(dataIn.delta1));
                        data.K15_2 = (0.8 * Math.Sqrt(data.gamma) + 6 * data.gamma) / DegToRad(dataIn.delta1);
                        data.K15 = Math.Min(1, data.K15_2);
                        data.K16 = 1.0 - 0.65 / (1 + Math.Pow(6 * data.gamma, 2)) *
                             Math.Sqrt(Math.PI / (3.0 * DegToRad(dataIn.delta1)));
                        data.K17 = 1.0 / (1.0 + 0.6 *
                             Math.Pow(dataIn.D / (dataIn.s - dataIn.c), 1.0 / 3.0) *
                             (dataIn.b / dataIn.D) * DegToRad(dataIn.delta1));
                        ...
                        data.v1_3 = -0.53 * data.K11 /
                                 (data.K14 * data.K16 * data.K17 * Math.Sin(0.5 * DegToRad(dataIn.delta1)));
                ...
                case SaddleType.SaddleWithoutRingWithSheet:
                    {
                        ...
                        data.K12 = (1.15 - 0.1432 * DegToRad(dataIn.delta2)) /
                                Math.Sin(0.5 * DegToRad(dataIn.delta2));
                        data.K13 = Math.Max(1.7 - 2.1 * DegToRad(dataIn.delta2) / Math.PI, 0) /
                                Math.Sin(0.5 * DegToRad(dataIn.delta2));
                        data.K14 = (1.45 - 0.43 * DegToRad(dataIn.delta2)) /
                                Math.Sin(0.5 * DegToRad(dataIn.delta2));
                        data.K15_2 = (0.8 * Math.Sqrt(data.gamma) + 6 * data.gamma) / DegToRad(dataIn.delta2);
                        data.K15 = Math.Min(1, data.K15_2);
                        data.K16 = 1 - 0.65 / (1 + Math.Pow(6 * data.gamma, 2)) *
                             Math.Sqrt(Math.PI / (3 * DegToRad(dataIn.delta2)));
                        data.K17 = 1.0 / (1.0 + 0.6 * Math.Pow(dataIn.D / data.sef, 1.0 / 3.0) *
                             (dataIn.b2 / dataIn.D) * DegToRad(dataIn.delta2));
                        data.sigma_mx = 4 * data.M1 / (Math.PI * Math.Pow(dataIn.D, 2) * data.sef);

                        data.v1_2 = -0.23 * data.K13 * data.K15 / (data.K12 * data.K10);
                        data.v1_3 = -0.53 * data.K11 / (data.K14 * data.K16 * data.K17 * Math.Sin(0.5 * DegToRad(dataIn.delta2)));
